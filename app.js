...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleLogin() {
    ...
    if (!isValidUser) {
      showNotification('Invalid credentials. Try demo@edufun.com / demo123', 'error');
      return;
    }
  ...
  function checkAnswer(btn, correctAnswer) {
    ...
    if (btn.textContent === correctAnswer) {
      ...
      if (mistakes >= 5) {
        showNotification('Too many mistakes! Resetting game.', 'error');
  ...
  document.getElementById('music-toggle').addEventListener('change', (e) => {
    ...
    saveSettings();
    showNotification(`Music ${e.target.checked ? 'On' : 'Off'}`, 'info');
  });
